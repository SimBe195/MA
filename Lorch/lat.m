...
/* TODO:
 * prime selection for neighbor method
...
{internal}
  return Vector(Eltseq(v`Element));
end intrinsic;
...
{The inner product (v,w)}
  ...
  F:= L`Form;
  o:= IsOrthogonal(L);
  require F cmpeq w`Parent`Form and o eq IsOrthogonal(w`Parent): "Incompatible arguments";
...
//    if L`Form ne A`Parent`Form then return false, "Incompatible lattices"; end if;
...
intrinsic HermitianLattice(M::ModDed, F::Mtrx, a::Map) -> LatModHerm
{The lattice consisting of the module M and the hermitian form F over the field K with Involution a}
  R:= BaseRing(M);
  require ISA(Type(R), RngOrd) : "The base ring must be an order in a number field";
  require Ncols(F) eq Dimension(M) : "The arguments must have the same dimensions";
  require IsField(Domain(a)) and IsField(Codomain(a)) : "The third argument is not a field automorphism";
  FF:= FieldOfFractions(R);
  ok, F:= CanChangeRing(F, FF);
  require ok: "The first two arguments must have the same base ring";

  ok, x:= IsCoercible(Domain(a), FF.2);
  require ok: "Base fields of arguments differ";
  ok, y:= IsCoercible(FF, a(x));
  require ok: "Base fields of arguments differ";
  a:= hom< FF -> FF | y >;
  require TestHerm(F, a) : "The second argument must be hermitian";
  X:= FixedField(FF, [a]);
  require Degree(FF, X) eq 2: "The third argument is not an involution";
  L:= New(LatMod);
  L`Module:= M;
  L`Form:= F;
  L`aut:= a;
...
{The lattice consisting of the module M and the hermitian form F over some quadratic field extension}
  R:= BaseRing(M);
  K:= FieldOfFractions(R);
  require Degree(K) eq 2: "Argument 3 must be a quadratic extension";
  a:= Automorphisms(K)[2];
  return HermitianLattice(M, F, a);
end intrinsic;

intrinsic HermitianLattice(B::Mtrx, F::Mtrx, a::Map) -> LatModHerm
{The hermitian standard lattice with basis B, form F and involution a}
  ...
  ok, B:= CanChangeRing(B, R);
  require ok : "Incompatible base rings";
  return HermitianLattice(Module(Rows(B)), F, a);
end intrinsic;
...
{The hermitian standard lattice with form F}
  K:= BaseRing(F);
  require IsField(K) and Degree(K) eq 2: "The base field of the form must be a quadratic extension";
  a:= Automorphisms(K)[2];
  assert a(K.1) ne K.1;
  return HermitianLattice(F^0, F, a);
end intrinsic;
*/
...
{The involution to the hermitian/ortohogonal space of L}
  return IsOrthogonal(L) select func< x | x > else Automorphisms(BaseRing(L`Form))[2];
end intrinsic;

intrinsic InternalConjugate(L::LatMod, X::Mtrx) -> Mtrx
{internal}
  if IsHermitian(L) then X:= Matrix( Ncols(X), [a(x): x in Eltseq(X) ]) where a:= Involution(L); end if;
  return Transpose(X);
end intrinsic;
...
intrinsic IsDefinite(L::LatMod) -> BoolElt
{Tests if the lattice L is (totally) definite}
  if not assigned L`Definite then
    L`Definite:= IsTotallyReal(BaseRing(L`Form)) and forall{ d: d in Diagonal( OrthogonalizeGram (L`Form) ) | IsTotallyPositive(d) };
  end if;
  return L`Definite;
end intrinsic;

intrinsic GramMatrix(L::LatMod) -> Mtrx, []
{The Gram matrix of a set of generators of L}
  B:= Generators(L`Module : Minimal);
  BM:= Matrix(B);
  return BM * L`Form * InternalConjugate(L, BM), B;
end intrinsic;
...
{Return a lattice over the integers if L is over QNF()}
  ...
  require IsOrthogonal(L) and IsDefinite(L) : "Only definite quadratic lattices are supported in Magma";
  return LatticeWithGram( LLLGram(ChangeRing(GramMatrix(L), Integers()) ));
end intrinsic;
...
{The discriminant ideal of L}
  if not assigned L`Discriminant then 
    ...
    T:= Matrix( [ p[2] : p in P ] );
    L`Discriminant:= Determinant(T)^2 * Determinant(L`Form) * (&*[ p[1]: p in P ])^2;
  end if;
...
{internal}
  return L1`Form cmpeq L2`Form and L1`Module eq L2`Module;
end intrinsic;

intrinsic 'subset'(L1::LatMod, L2::LatMod) -> BoolElt
{internal}
  require L1`Form cmpeq L2`Form : "Incompatible arguments";
  return L1`Module subset L2`Module;
end intrinsic;

function GetGens(L, x)
  if Type(x) eq LatMod then x:= Generators(x); end if;
...
{The quotient of L by RHS}
  try 
    ...
    require false : "The right hand side is not a sublattice of the left hand side";
  end try;
  //TODO:
...
{internal}
  require L`Form cmpeq M`Form : "The arguments are not compatible";
  LL:= New(Type(L));
  LL`Form:= L`Form;
  LL`Module:= L`Module + M`Module;
  return LL;
end intrinsic;
...
{internal}
  ...
  LL:= New(Type(L));
  LL`Form:= L`Form;
  LL`Module:= L`Module meet M`Module;
  return LL;
end intrinsic;
...
{The dual lattice of L}
  ...
  GI:= G^-1;
  BB:= [ &+[ GI[i,j]  * B[j] : j in [1..#P ]] : i in [1..#P ]];
  a:= Involution(L);
  LL:= New(Type(L));
  LL`Module:= Module( [ < a(P[i,1])^-1, BB[i] > : i in [1..#P] ] );
  LL`Form:= L`Form;
  return LL;
end intrinsic;
...
// This breaks the AutomorphismGroup code which assumes the ZBasis B is of a certain form.
//    G, T:= LLLGram(F[1]: Delta:= 0.999, Eta:= 0.501);
//    BB:=  Matrix(FieldOfFractions(R), T)*B;
//    GG:= BB * L`Form * InternalConjugate(L, BB);
//    G * d[1] eq Matrix( Ncols(G), [ AbsoluteTrace(e) : e in Eltseq( GG ) ] );
//    L`ZForms:= < [G, T * F[2] * Transpose(T)], d, Matrix(FieldOfFractions(R), T)*B, a >;
    L`ZForms:= < F, d, B, a >;
  end if;
...
{Returns all elements of norm a in L up to sign}
  ...
  if not IsTotallyPositive(a) or a notin Norm(L) then return [ L | ]; end if;

  Z:= ZBasis(L); d:= #Z; n:= AbsoluteDegree(FF);
  B:= Matrix([ Eltseq(x) : x in Z ]);
  // We solve xGx^t = 1 where
  G:= a^-1 * B * L`Form * InternalConjugate(L, B);
  LL:= LatticeWithGram( Matrix(Ncols(G), [ AbsoluteTrace(x) : x in Eltseq(G) ]) );

  // The easy case.
  if ideal< R | a > eq Norm(L) then
    S:= Minimum(LL) eq n select ShortestVectors(LL) else [];
  else
    S:= ShortVectorsMatrix(LL, n, n);
    SF:= Matrix(FF, S);
    SG:= SF * G;
    S:= [ S[i]: i in [1..Nrows(S)] | IsOne(InnerProduct(SG[i], SF[i])) ];
  end if;
  return IsEmpty(S) select [ L | ] else [ &+[ s[i] * Z[i] : i in [1..d] | s[i] ne 0 ] : s in S ];
end intrinsic;

intrinsic AutomorphismGroup(L::LatMod) -> GrpMat
{The automorphism group of the lattice L}
  ...
  if not assigned L`Auto then
    ...
    d:= Dimension(L);
    Idx:= [1..m by (m div d)];
    BF:= RowSubmatrix(B, Idx)^-1;
    Gens:= [];
    for i in [1..Ngens(A)] do
      a:= RowSubmatrix(A.i, Idx);
      Append(~Gens, BF * Matrix([ &+[ B[j] * a[i,j] : j in [1..m] | a[i,j] ne 0 ] : i in [1..d] ]));
    end for;

    L`Auto:= MatrixGroup< d, BaseRing(BF) | Gens >;
    assert #L`Auto eq #A;
  end if;
  return L`Auto;
end intrinsic;

intrinsic IsIsometric(L1::LatMod, L2::LatMod : Isometry, Check) -> BoolElt, Mtrx
{Tests if the lattices L1 and L2 are isometric}
  ...
  require Rank(L1) eq Rank(L2) : "The lattices must have the same rank";
  require IsOrthogonal(L1) eq IsOrthogonal(L2) : "The lattices must either both be orthogonal or hermitian";
  require IsDefinite(L1) and IsDefinite(L2) : "Only implemented for definite lattices";

  if (assigned L1`Auto and assigned L2`Auto and #L1`Auto ne #L2`Auto) or 
  ...
  if not (Isometry and result) then return result, _; end if;
  T:= T2^-1 * T * T1;

  m:= Nrows(B1); 
  d:= Dimension(L1);
  Idx:= [1..m by (m div d)];
  S:= RowSubmatrix(B2, Idx)^-1 * Matrix(BaseRing(B1), RowSubmatrix(T, Idx)) * B1;
  if Check then
    assert S * L1`Form * InternalConjugate(L1, S) eq L2`Form;
    assert Module(Rows(Matrix(Generators(L2`Module)) * S)) eq L1`Module;
  end if;
  return true, S;
end intrinsic;
...
{The norm of the lattice L}
  ...
  if not assigned L`Norm then
    ...
    L`Norm:= &+{ F[i,i] * P[i,1]^2 : i in [1..#P] } + 2*Scale(L);
  end if;
  return L`Norm;
end intrinsic;
...
{The scale of the lattice L}
  ...
  if not assigned L`Scale then
    ...
    F:= B * L`Form * InternalConjugate(L, B);
    a:= Involution(L);
    L`Scale:= &+{ F[i,j] * P[i,1] * a(P[j,1]) : i in [1..j], j in [1..#P] | F[i,j] ne 0 };
  end if;
  return L`Scale;
end intrinsic;
...
//Hasse:= func< D, p | &*[ Integers() | HS(D[i], D[j], p) : j in [i+1..n], i in [1..n] ] where n:= #D >;
Hasse:= func< D, p | &*[ Integers() | HS(D[i], &*D[ [i+1..n] ], p) : i in [1..n-1] ] where n:= #D >;

function MyDiagonal(L)
  if not assigned L`Diagonal then
    L`Diagonal:= Diagonal(OrthogonalizeGram(L`Form));
  end if;
  return L`Diagonal;
end function;

intrinsic HasseInvariant(L::LatMod, p::RngOrdIdl) -> RngIntElt
{The Hasse invariant of L at p}
  ...
  return Hasse(MyDiagonal(L), p);
end intrinsic;

intrinsic WittInvariant(L::LatMod, p::RngOrdIdl) -> RngIntElt
{The Witt invariant of L at p}
  R:= Order(p);
  require R cmpeq BaseRing(L) and IsPrime(p):
    "The second argument must be a prime ideal of the base ring of the lattice";
  h:= Hasse(MyDiagonal(L), p);
  Det:= Determinant(L`Form);
  K:= NumberField(R);
...
intrinsic IsRationallyEquivalent(L1::LatMod, L2::LatMod, p::RngOrdIdl) -> BoolElt
{Tests if L1 and L2 are equivalent over the completion of their base field at p}
  ...
  require IsOrthogonal(L1) and IsOrthogonal(L2): "Non-orthogonal spaces";
  R:= BaseRing(L1);
  ...
  return Type(QuadraticDefect( Determinant(L1`Form) * Determinant(L2`Form), p )) eq Infty and 
         HasseInvariant(L1, p) eq HasseInvariant(L2, p);
end intrinsic;

intrinsic IsRationallyEquivalent(L1::LatMod, L2::LatMod) -> BoolElt
{Tests if L1 and L2 are equivalent over their base field}
  if L1`Form cmpeq L2`Form then return true; end if;
  require BaseRing(L1) cmpeq BaseRing(L2): "Incompatible lattices";
  require IsOrthogonal(L1) and IsOrthogonal(L2): "Non-orthogonal spaces";

  Det1, F1, I1:= QuadraticFormInvariants(L1`Form);
  Det2, F2, I2:= QuadraticFormInvariants(L2`Form);
  return I1 eq I2 and F1 eq F2 and IsSquare(Det1*Det2);
end intrinsic;
...
my_little_helper := function(G1, G2, p)
  ...
  return &*[MyHilbertSymbol(G1o[i], G1o[j], p): i in [1..j-1], j in [1..#G1o]]
      eq &*[MyHilbertSymbol(G2o[i], G2o[j], p): i in [1..j-1], j in [1..#G2o]];
end function;
...
{Returns true if and only if the p-adic completions of L1 and L2 are isometric}
  ...
  if Dimension(L1) ne Dimension(L2) then return false; end if;
...
function JordanSplitting(M, F, p)
  O:= Order(p);
  even:= Minimum(p) eq 2;
...
{A Jordan decomposition of the completion of L at p}
  ...
  return JordanSplitting(L`Module, L`Form, p); 
end intrinsic;
...
intrinsic Neighbours(L::LatMod, p::RngOrdIdl : TestGenus2:= false, AutoOrbits:= false, CallBack:= false, Max:= Infinity()) -> []
{The p-neighbours of L}
  R:= BaseRing(L);
  require R cmpeq Order(p): "The arguments are incompatible";
  require IsPrime(p): "The second argument must be a prime ideal";
//  require p notin BadPrimes(L) : "The lattice must be p-adically integral and unimodular";
  if IsOrthogonal(L) then
    e:= Valuation(R ! 2, p);
  ...
  if AutoOrbits cmpne false then
    ...
    A:= [ Matrix(n, [ x @ h : x in Eltseq(M * A.i * MI) ]) : i in [1..Ngens(A)] ] where MI:= M^-1;
...
//  if i mod 100000 eq 0 then i; end if;
    ...
    V:= VV * M;
    LL:= LatticeModule(pM + Module(Rows(V)), L`Form);
    assert not TestGenus2 or forall{p: p in P2 | IsLocallyIsometric(L, LL, p) };
...
function GetPrimes(L, Ignore)
  assert Rank(L) ge 3;		// so unimodular primes are OK
  Ps:= BadPrimes(L);
  Extra:= [];
  for p in Ps do
    J:= JordanDecomposition(L, p);
    d:= Max( { Nrows(j) : j in J } );
    even:= Minimum(p) eq 2;
    if (not even and d ge 2) or (even and d ge 3) then continue; end if;
    if not Ignore then error "implement me"; end if;
  end for;

  R:= BaseRing(L);
  Bad:= IsEmpty(Ps) select 2*R else 2*&*Ps;
  C, h:= ClassGroup(R);		// The class group should do.
  //C, h:= NarrowClassGroup(R);
  Classes:= [ c : c in C ];
  Found:= []; p:= 2;
  while #[f: f in Found] lt #Classes do
    p:= NextPrime(p);
    D:= Decomposition(R, p);
    for d in D do
      i:= Index( Classes, (d[1]) @@ h );
      if Valuation(Bad, d[1]) eq 0 and not IsDefined(Found, i) then Found[i]:= d[1]; end if;
    end for;
  end while;

  M:= Max([ Norm(f) : f in Found ]);
  P:= PrimesUpTo(M-1, NumberField(R) : coprime_to:= Bad);
  for p in P do
    i:= Index(Classes, p @@ h );
    if Norm(Found[i]) gt Norm(p) then Found[i]:= p; end if;
  end for;

  return Extra cat Found;
end function;
...
intrinsic GenusRepresentatives(L::LatMod : Max:= Infinity(), UseAuto:= false, Extra:= 0, Ignore:= false, TestGenus2:= false) -> []
{Representatives of the isometry classes of the genus of L}
  require Rank(L) ge 3 : "The rank of L must be at least 3";
  requirege Extra, 0;
  Primes:= GetPrimes(L, Ignore);
  Reps:= [ L ];
  extra:= false;
  ...
  while true do
    ...
    while i le #Reps and #Reps lt Max do
      for p in Primes do
//        <i, Norm(p) >;
        N:= Neighbours(Reps[i], p: AutoOrbits:= UseAuto, TestGenus2:= TestGenus2);
#N, "candidates";
        for n in N do
          if forall{ r: r in Reps | not IsIsometric(r, n) } then 
	    error if not Ignore and extra, "Oups, found new rep in the extra rounds! --- Please report";
  	    Append(~Reps, n);
"now at", #Reps;
...
intrinsic IsOneClassGenus(L::LatMod : Extra:= 0, UseAuto:= false, Ignore:= false, TestGenus2:= false) -> BoolElt
{Returns true if the genus of L has class number one}
  Primes:= GetPrimes(L, Ignore);
  call:= func< LL | not iso, iso where iso:= IsIsometric(L, LL) >;
  for p in Primes do
    if #Neighbours(L, p: CallBack:= call, AutoOrbits:= UseAuto, TestGenus2:= TestGenus2) gt 0 then return false; end if;
  end for;
  return true;
//  return #GenusRepresentatives(L : Max:= 2, Extra:= Extra, UseAuto:= UseAuto, Ignore:= Ignore, TestGenus2:= TestGenus2) eq 1;
end intrinsic;
...
{The maximal sublattices of L that contain pL}
  ...
  if AutoOrbits cmpne false then
    A:= AutoOrbits cmpeq true select AutomorphismGroup(L) else ChangeRing(AutoOrbits, F);
    BMI:= BM^-1;
    AT:= sub< GL(n, k) | [Transpose(Matrix(k, n, [ x @ h : x in Eltseq(BM * A.i * BMI) ])) : i in [1..Ngens(A)]] >;
    AutoOrbits:= exists{i: i in [1..Ngens(AT)] | not IsScalar(AT.i) };
  end if;
  if AutoOrbits then
    Ls:= IsPrimeField(k) select [ l[2].1: l in OrbitsOfSpaces(AT, 1) ] else [ l[1].1: l in LineOrbits(AT) ];
  else
  ...
  for s in Ls do
    if AutoOrbits then
      m:= ChangeRing(KernelMatrix( Matrix(1, Eltseq(s)) ), hinv);
    else
      m:= ChangeRing(s, hinv);
    end if;
    m:= ChangeRing(m, F);
    LL:= orth select LatticeModule(Module(Rows(m * BM) cat Gens), L`Form) else HermitianLattice(Module(Rows(m * BM) cat Gens), L`Form);
...
{The sublattices of index p^i in L that contain pL}
  ...
  require AutoOrbits cmpne false: "UseAuto";
  A:= AutoOrbits cmpeq true select AutomorphismGroup(L) else ChangeRing(AutoOrbits, F);
  BMI:= BM^-1;
  A:= sub< GL(n, k) | [Matrix(k, n, [ x @ h : x in Eltseq(BM * A.i * BMI) ]) : i in [1..Ngens(A)]] >;
  Ls:= [ l[2]: l in OrbitsOfSpaces(A, n-i) ];

  Gens:= Generators(p*L`Module);
  Result:= []; keep:= true; cont:= true;
  for s in Ls do
    m:= ChangeRing(BasisMatrix(s), hinv);
    m:= ChangeRing(m, F);
    LL:= LatticeModule(Module(Rows(m * BM) cat Gens), L`Form);
    if CallBack cmpne false then
      keep, cont:= CallBack(LL);
      if keep then Append(~Result, LL); end if;
      if not cont then break; end if;
...
{The maximal sublattices of L that contain pL}
  ...
  Gens:= Generators(L`Module);
  Result:= []; keep:= true; cont:= true;
  for l in Ls do
//    l:= AutoOrbits select Ls[i] else Next(Ls);
    m:= pi * ChangeRing(ChangeRing(l, hinv), F);
    LL:= orth select LatticeModule(Module(Append(Gens, m*BM)), L`Form) else HermitianLattice(Module(Append(Gens, m*BM)), L`Form);
    if CallBack cmpne false then
      keep, cont:= CallBack(LL);
      if keep then Append(~Result, LL); end if;
      if not cont then break; end if;
...
intrinsic IsMaximalIntegral(L::LatMod, p::RngOrdIdl) -> BoolElt, ModTupFldElt
{Checks whether L is p-maximal integral. If not, a minimal integral over-lattice at p is returned}
  ...
  G:= BM * 2 * F * Transpose(BM);
  V:= KernelMatrix(Matrix(Degree(L), [x @ h: x in Eltseq(G)]));
...
/*
  if Minimum(p) ne 2 then
    VV:= Matrix(FF, Ncols(V), Eltseq(V) @@ h);
    GG:= VV * G * Transpose(VV) / PrimitiveElement(p);
    GG:= Matrix( 
...
*/
  ...
  while not IsZero(x) do
    ...
    if valv ge val2+2 then return false, pi*v*BM; end if;
    x:= Next(PP);
  end while;
  return true, _;
end intrinsic;
...
{A lattice which is maximal integral with respect to the quadratic form Q}
  ...
  F:= InnerProductMatrix(L);
  FD:= Factorization(2^n * Discriminant(L));
//  FD:= [ f: f in Factorization(2^n * Discriminant(L)) | f[2] ge 2 ];
  for d in FD do
    p, val:= Explode(d);
    if val ge 2 then
    repeat
      ok, x:= IsMaximalIntegral(L, p);
      if not ok then 
        L:= LatticeModule( Module(L) + Module([x]), F ); 
        val -:= 2;
        ok:= val le 1;
      end if;
    until ok;
...
//    assert GuessMaxDet(L, p) eq Valuation( Discriminant(L), p);
    if GuessMaxDet(L, p) ne Valuation( Discriminant(L), p) then print "WRONG determinant guess. -> Please report.", p; end if;
  end for;
  return L;

  X:= L`Module;
  delete L;
  detQ:= Determinant(Q) * 2^n;
  for d in FD do
    P, val:= Explode(d);
    if val le 1 then continue; end if;
    k, h:= ResidueClassField(P);
    pi:= WeakApproximation([P], [-1]);
    val2:= Valuation(R ! 2, P);

    repeat
      BM:= Matrix(LocalBasis(X, P: Type:= "Submodule"));
      G:= BM * 2 * Q * Transpose(BM);
      V:= KernelMatrix(Matrix(n, [x @ h: x in Eltseq(G)]));
      if Nrows(V) eq 0 then continue d; end if;
      leave:= true;
      for x in MyPSReps(k, Nrows(V)-1) do
        e:= Eltseq(x * V) @@ h;
        v:= Vector(F, e);
        valv:= Valuation( ((v*G) * Matrix(1,e))[1], P );
        assert valv ge 1;
        // TODO: Better code depending on whether -1 is a square or not and then take (1,p) or (p,p)
        if valv lt val2+2 then continue; end if;
        w:= pi*v*BM;
	...
	// TODO: Remove this test when done...
        tmp:= Matrix(LocalBasis(X, P));
        assert Valuation( Determinant(tmp)^2 * detQ, P) eq val;
...
IsLocalSquare:= func< a, p | Type(QuadraticDefect(a, p)) eq Infty >;
...
// We already take care of Nr(disc(E/K))^((m-1)/2) globally.
  if IsEven(m) and IsOdd(Valuation(Determinant(L`Form), p)) then
    e +:= (m-1)/2;
  end if;
  assert IsIntegral(e);
  e:= Integers() ! e;
...
{Returns the mass of L}
  ...
  m:= Rank(L);
  assert m eq Ncols(L`Form);
  r:= m div 2;
  Det, Hasse:= QuadraticFormInvariants(L`Form : Minimize:= false);
  ...
  if IsOdd(m) then
    ...
    for p in Witt do
      w:= Type(QuadraticDefect(Disc, p)) eq Infty select -1 else 1;
...
{The invariants describing the quadratic form M}
  ...
  F:= Minimize select { p : p in P | Hasse(D, p) eq -1 } else { <p, Hasse(D, p) > : p in P };
...
{Computes a quadratic form with of dimension Dim and determinant Det that has Hasse invariants -1 at the primes in Finite.
 The number of negative entries of the real signature is given in Negative}
  ...
  if Dim eq 2 then
    ok:= forall(p){p: p in Finite | Type(QuadraticDefect(-Det, p)) ne Infty};
 ...
 The number of negative entries of the i-th real signature is given in Negative[i]}
  ...
  if Dim ge 4 then
    ...
    while Dim ge 4 do
      ...
      s:= RealSigns(x);
      k:= Min(Dim-3, [ s[i] eq 1 select (Dim - Negative[i]) else (Negative[i]) : i in [1..#Negative] ]);
      D2 cat:= [ (K ! x)^^k ];
      Dim -:= k;
      for i in [1..#Negative] do
    ...
    PP:= { p[1] : p in Factorization(R*x), x in Append(D2, 2) } join { p[1] : p in Factorization(Det) };
    Finite:= { p: p in PP | HS(d, -Det, p) * (p in f select -1 else 1) * (p in Finite select -1 else 1) eq -1 };
    D cat:= D2;
...
//  "Dim 3", Det;
    // The primes at which the form is anisotropic
    PP:= Finite join { f[1]: f in Factorization(R*(2*Det)) };
    ...
    PP:= { p[1] : p in Factorization(R*x), x in [K ! 2, Det, a] };
    Finite:= { p: p in PP | HS(a, -Det, p) * (p in Finite select -1 else 1) eq -1 };
  ...
  // The binary case
  PP:= [ p[1]: p in Factorization((2*Det)*R) | p[1] notin Finite ];	// the places at which we need +1
...
//  return &*[ Integers() | HS(D[i], D[j], p) : i in [1..j-1], j in [1..#D] ];
  return Hasse(D, p);
end function;
